...
export default class DataView extends Vue {
  @Prop({ type: Boolean, default: true }) readonly showCheckbox //    是否显示操作按钮
  ...
  created() {
    this.$nextTick(() => {
      this.$refs.orgTree.setCurrentKey(this.data[0].zhujian); 
  })
  }
  nodeClick(data, node) {
  ...
  handleCheck(data,checked,indeterminate){
    ...
    for(var i=0 ;i<filterNodes.length;i++){
      if(filterNodes[i].parent){
        let  pnode= this.$refs.orgTree.getNode(filterNodes[i].parent)
